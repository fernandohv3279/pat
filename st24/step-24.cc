...
namespace Step24
{
    ...
    , wave_speed(1)
  {
    ...
    for (double detector_angle = 0; detector_angle <= 2*numbers::PI+(0.01/180);
         detector_angle += detector_step_angle / 180 * numbers::PI)
	   {
	double detector_angle_grad= detector_angle * 180 /numbers::PI;
	std::cout <<"detector angle " << detector_angle_grad <<std::endl;
  ...
  {
    DataOut<dim> data_outInitial;

    data_outInitial.attach_dof_handler(dof_handler);
    data_outInitial.add_data_vector(old_solution_p, "P");

    data_outInitial.build_patches();

    std::ofstream outputInitial("solution-000.vtu");
    data_outInitial.write_vtu(outputInitial);
    std::cout << "Initial condition saved"<< std::endl;
  }
    ///////////////////////////////////////////////////////////
    old_solution_v = 0;
